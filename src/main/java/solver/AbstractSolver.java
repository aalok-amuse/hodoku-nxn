...
//    private SudokuSet tmpSet = new SudokuSet();
	...
	/**
	 * Tracks the cell count this solver's size-dependent buffers were sized for.
	 * Used by {@link #ensureBuffersSized(Sudoku2)} to decide whether to (re)allocate.
	 * Subclasses that override {@link #onSudokuChanged(Sudoku2)} consult / reset this.
	 */
	protected int sizedForLength = -1;

	/**
	 * Called by callers (e.g. {@code getStep()}) when the active sudoku is set or
	 * may have changed spec. Subclasses with size-dependent buffers override this
	 * to (re)allocate them when {@code sudoku.getLength() != sizedForLength}.
	 *
	 * <p>Default: no-op (solvers without size-dependent buffers).</p>
	 *
	 * @param sudoku the sudoku that is now active
	 */
	...
	/**
	 * Convenience hook subclasses can call from {@code getStep()}.
	 * Triggers {@link #onSudokuChanged(Sudoku2)} once per spec change.
	 */
