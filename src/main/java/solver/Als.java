...
public class Als {
	...
	public void computeFields(SudokuStepFinder finder) {
		...
		for (int i = 1; i <= finder.getSudoku().getUnitCount(); i++) {
			if ((candidates & finder.getSudoku().getMasksArr()[i]) != 0) {
				...
				finder.getSudoku().getSpecData().getBuddies(indicesPerCandidat[i], buddiesPerCandidat[i]);
	...
	public int getChainPenalty() {
		if (chainPenalty == -1) {
			chainPenalty = getChainPenalty(Integer.bitCount(candidates & 0xFFFF));
		}
		return chainPenalty;
	}
