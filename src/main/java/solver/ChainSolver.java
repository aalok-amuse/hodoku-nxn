...
public class ChainSolver extends AbstractSolver {
	 ...
	 * (index cellIndex * 17 + candidate).
	 */
	private int[] startIndices = new int[4352];
	/**
	 * The end indices + 1 in {@link #links} for all cell/candidate combinations
	 * (index cellIndex * 17 + candidate).
	 */
	private int[] endIndices = new int[4352];
...
//        System.out.println("getChains(" + type + ")");
		...
		for (startIndex = 0; startIndex < sudoku.getCells().length; startIndex++) {
			...
			for (int i = 0; i < startCandidates.length; i++) {
				...
				int linkStartIndex = startIndex * 17 + startCandidate;
...
//                        }
					...
					entry.aktIndex = startIndices[entry.cellIndex * 17 + entry.candidate];
					entry.endIndex = endIndices[entry.cellIndex * 17 + entry.candidate];
					chainSet.clear();
					chainSet.add(startIndex);
					// prepare the sets: all instances of startCandidate that can see the cell at
					// startIndex
					startCellSetM1 = sudoku.getSpecData().buddiesM1[startIndex] & finder.getCandidates()[startCandidate].getMask1();
					startCellSetM2 = sudoku.getSpecData().buddiesM2[startIndex] & finder.getCandidates()[startCandidate].getMask2();
					if (type == REMOTE_PAIR) {
						// the same for the second candidate in a RP
						startCellSet2M1 = sudoku.getSpecData().buddiesM1[startIndex]
								& finder.getCandidates()[startCandidate2].getMask1();
						startCellSet2M2 = sudoku.getSpecData().buddiesM2[startIndex]
...
//                chainSet.remove(Chain.getSCellIndex(chain[stackLevel]));
			...
			if (typ == NICE_LOOP) {
				...
				if (stackLevel > 1 && newLinkIsStrong && newLinkCandidate == startCandidate) {
					...
					long m1 = startCellSetM1 & sudoku.getSpecData().buddiesM1[newLinkIndex];
					long m2 = startCellSetM2 & sudoku.getSpecData().buddiesM2[newLinkIndex];
		...
		if (stackLevel > 7) {
			...
			for (int i = 0; i <= stackLevel; i += 2) {
				...
				for (int j = i + 6; j <= stackLevel; j += 4) {
					rpTmp.set(sudoku.getSpecData().buddies[Chain.getSCellIndex(chain[i])]);
					rpTmp.and(sudoku.getSpecData().buddies[Chain.getSCellIndex(chain[j])]);
			...
			long m21 = startCellSet2M1 & sudoku.getSpecData().buddiesM1[endIndex];
			long m22 = startCellSet2M2 & sudoku.getSpecData().buddiesM2[endIndex];
			rpCand1.set(m1, m2);
			rpCand2.set(m21, m22);
		}
		// no create the step
		...
		if (!firstLinkStrong && !lastLinkStrong && startCandidate == endCandidate) {
			...
			for (int i = 1; i <= chainIndex; i++) {
				...
				if (!Chain.isSStrong(chain[i]) && Chain.getSCellIndex(chain[i - 1]) != Chain.getSCellIndex(chain[i])) {
					...
					checkBuddies.set(sudoku.getSpecData().buddies[Chain.getSCellIndex(chain[i - 1])]);
					// checkBuddies.and(sudoku.buddies[(chain[i] / 10) % 100]);
					checkBuddies.and(sudoku.getSpecData().buddies[Chain.getSCellIndex(chain[i])]);
...
//            }
	...
	private void getAllLinks(int type) {
		...
		for (int cellIndex = 0; cellIndex < sudoku.getCells().length; cellIndex++) {
			...
			if (cell == 0 || ((type == REMOTE_PAIR || type == XY_CHAIN) && sudoku.getSpecData().anzValues[cell] != 2)) {
				// ignore filled cells
				continue;
			}
			for (int cellCandidate = 1; cellCandidate <= sudoku.getUnitCount(); cellCandidate++) {
//                System.out.println(cellIndex + "/" + cellCandidate);
				startEndIndex = cellIndex * 17 + cellCandidate;
//                if (sudoku.getValue(cellIndex) != 0 || !sudoku.isCandidate(cellIndex, cellCandidate)) {
				...
				int[] cands = sudoku.getSpecData().possibleValues[cell];
				if (sudoku.getSpecData().anzValues[cell] == 2) {
					// bivalue cell
					if (type != X_CHAIN && type != TURBOT_FISH) {
						int cand = cands[0];
...
//                        System.out.println("  " + index + ": " + cellIndex + "/" + cands[k] + "/" + false);
				...
				for (int constr = 0; constr < sudoku.getConstraintsArr()[cellIndex].length; constr++) {
					boolean strong = false;
					if (free[sudoku.getConstraintsArr()[cellIndex][constr]][cellCandidate] == 2) {
						// strong link!
						if (type == X_CHAIN || type == TURBOT_FISH || type == NICE_LOOP) {
							strong = true;
						}
					}
					int[] indices = sudoku.getAllUnitsArr()[sudoku.getConstraintsArr()[cellIndex][constr]];
					for (int k = 0; k < indices.length; k++) {
						if (indices[k] != cellIndex && sudoku.isCandidate(indices[k], cellCandidate)) {
							...
							if (type == XY_CHAIN && sudoku.getSpecData().anzValues[cell2] != 2) {
								// must be bivalue
								continue;
							}
							if (constr == 2 && (sudoku.rowOf(cellIndex) == sudoku.rowOf(indices[k])
									|| sudoku.colOf(cellIndex) == sudoku.colOf(indices[k]))) {
