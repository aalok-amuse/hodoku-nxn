...
public class ColoringSolver extends AbstractSolver {
	 ...
	 * for candidate x.
	 */
	private SudokuSet[][][] sets = new SudokuSet[17][MAX_COLOR][2];
...
//        Logger.getLogger(getClass().getName()).log(Level.FINE, "end of findAllMultiColors() (" + ticks + "ms)");
	...
	private boolean checkColorWrap(SudokuSet set) {
		for (int i = 0; i < set.size() - 1; i++) {
			for (int j = i + 1; j < set.size(); j++) {
				if (sudoku.getSpecData().buddies[set.get(i)].contains(set.get(j))) {
					return true;
				}
			}
		}
	...
	private void checkCandidateToDelete(SudokuSet set1, SudokuSet set2, int cand) {
		...
		for (int i = 0; i < set1.size(); i++) {
			for (int j = 0; j < set2.size(); j++) {
				tmpSet1.set(sudoku.getSpecData().buddies[set1.get(i)]);
				tmpSet1.and(sudoku.getSpecData().buddies[set2.get(j)]);
...
//                }
	...
	private boolean checkMultiColor1(SudokuSet set, SudokuSet s21, SudokuSet s22) {
		...
		for (int i = 0; i < set.size(); i++) {
			tmpSet1.set(sudoku.getSpecData().buddies[set.get(i)]);
	...
	private boolean checkMultiColor2(SudokuSet set1, SudokuSet set2) {
		for (int i = 0; i < set1.size(); i++) {
			for (int j = 0; j < set2.size(); j++) {
				if (sudoku.getSpecData().buddies[set1.get(i)].contains(set2.get(j))) {
					return true;
				}
			}
		}
	...
	private int doColoring(int cand) {
		...
		for (int i = 0; i < size; i++) {
			int index = values[i];
			if (free[sudoku.getConstraintsArr()[index][0]][cand] != 2 && free[sudoku.getConstraintsArr()[index][1]][cand] != 2
					&& free[sudoku.getConstraintsArr()[index][2]][cand] != 2) {
	...
	private void doColoringForColorRecursive(int index, int cand, boolean on) {
		...
		// recursion
		doColoringForColorRecursive(getConjugateIndex(index, cand, sudoku.getConstraintsArr()[index][0]), cand, !on);
		doColoringForColorRecursive(getConjugateIndex(index, cand, sudoku.getConstraintsArr()[index][1]), cand, !on);
		doColoringForColorRecursive(getConjugateIndex(index, cand, sudoku.getConstraintsArr()[index][2]), cand, !on);
	}
	...
	private int getConjugateIndex(int index, int cand, int constraint) {
		...
		tmpSet1.and(sudoku.getSpecData().allConstraintsTemplates[constraint]);
