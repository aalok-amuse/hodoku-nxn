...
public class FishSolver extends AbstractSolver {
	...
	/** True for all base units that are currently used. Sized per spec. */
	private boolean[] baseUnitsUsed;
	/** The recursion stack for the base unit search. Sized per spec. */
	private BaseStackEntry[] baseStack;
	/** The index of the current level in the {@link #baseStack}. */
	private int baseLevel = 0;
	/** True for all cover units that are currently used. Sized per spec. */
	private boolean[] coverUnitsUsed;
	/** The recursion stack for the cover unit search. Sized per spec. */
	...
	protected void onSudokuChanged(Sudoku2 sudoku) {
		...
		int u3 = n * 3;
		baseUnits = new int[u3];
		baseCandidatesM1 = new long[u3];
		baseCandidatesM2 = new long[u3];
		allCoverUnits = new int[u3];
		allCoverCandidatesM1 = new long[u3];
		allCoverCandidatesM2 = new long[u3];
		coverUnits = new int[u3];
		coverCandidatesM1 = new long[u3];
		coverCandidatesM2 = new long[u3];
		baseUnitsUsed = new boolean[u3];
		coverUnitsUsed = new boolean[u3];
		baseStack = new BaseStackEntry[n];
		coverStack = new CoverStackEntry[n];
		for (int i = 0; i < n; i++) {
...
//            if (doTemplates && ! entry.endoFins.isEmpty()) {
			if (doTemplates && (bEntry.endoFinsM1 != 0 || bEntry.endoFinsM2 != 0)) {
				...
				sudoku.getSpecData().getBuddies9x9(bEntry.endoFinsM1, bEntry.endoFinsM2, getBuddiesSet);
...
//                    sudoku.getSpecData().getBuddies(fins, finBuddies);
					sudoku.getSpecData().getBuddies9x9(finsM1, finsM2, getBuddiesSet);
...
//            printSet("tmpSetM1 with fins", tmpSetM1, tmpSetM2);
			...
			krakenFinSet.clear();
			for (int endCandidate = 1; endCandidate <= sudoku.getUnitCount(); endCandidate++) {
...
//                    checkSashimiSet.andNot(finSet);
		...
		for (int i = 0; i < baseUnitsUsed.length; i++) {
			if (baseUnitsUsed[i]) {
				globalStep.addBaseEntity(sudoku.getSpecData().constraintType[i],
						sudoku.getSpecData().constraintNumber[i]);
			}
		}
		for (int i = 0; i < coverUnitsUsed.length; i++) {
			if (coverUnitsUsed[i]) {
				globalStep.addCoverEntity(sudoku.getSpecData().constraintType[i],
						sudoku.getSpecData().constraintNumber[i]);
			}
		}
	...
	private int getUnitMask(boolean[] used) {
		...
		for (int i = 0; i < used.length; i++) {
			if (used[i]) {
				mask |= MASKS[sudoku.getSpecData().constraintType[i]];
			}
		}
		return mask;
	}
...
//            tmpSet.set(sudoku.getSpecData().allConstraintsTemplates[i]);
//            tmpSet.and(candidates);
			tmpSetM1 = sudoku.getSpecData().allConstraintsTemplatesM1[i] & candidatesM1;
			tmpSetM2 = sudoku.getSpecData().allConstraintsTemplatesM2[i] & candidatesM2;
//            if (tmpSet.isEmpty()) {
