...
public class MiscellaneousSolver extends AbstractSolver {
	...
	protected void onSudokuChanged(Sudoku2 sudoku) {
		...
		stack1 = new StackEntry[n];
		stack2 = new StackEntry[n];
		for (int i = 0; i < n; i++) {
	...
	protected List<SolutionStep> getAllSueDeCoqs() {
		...
		getSueDeCoqInt(sudoku.getSpecData().rowTemplates, sudoku.getSpecData().blockTemplates, false);
		getSueDeCoqInt(sudoku.getSpecData().colTemplates, sudoku.getSpecData().blockTemplates, false);
	...
	private SolutionStep getSueDeCoq(boolean onlyOne) {
		SolutionStep step = getSueDeCoqInt(sudoku.getSpecData().rowTemplates, sudoku.getSpecData().blockTemplates, onlyOne);
		if (onlyOne && step != null) {
			return step;
		}
		return getSueDeCoqInt(sudoku.getSpecData().colTemplates, sudoku.getSpecData().blockTemplates, onlyOne);
	}
	...
	private SolutionStep checkIntersection(boolean onlyOne) {
		...
		for (int i1 = 0; i1 < max - 1; i1++) {
			...
			for (int i2 = i1 + 1; i2 < max; i2++) {
				...
				nPlus = sudoku.getSpecData().anzValues[cand2] - 2;
				if (nPlus >= 2) {
					// possible SDC -> check
				...
				for (int i3 = i2 + 1; i3 < max; i3++) {
					...
					nPlus = sudoku.getSpecData().anzValues[cand3] - 3;
					if (nPlus >= 2) {
						// possible SDC -> check
	...
	private SolutionStep checkHouses(int nPlus, short cand, boolean onlyOne) {
		...
		return checkHouses(nPlus, nonBlockSourceSet, sudoku.getMaxMask(), onlyOne, false);
	}
	...
	private SolutionStep checkHouses(int nPlus, SudokuSet sourceSet, short allowedCandSet, boolean onlyOne,
		...
		while (true) {
			...
			if ((stack[level].candidates & ~allowedCandSet) == 0) {
				...
				int anzContained = sudoku.getSpecData().anzValues[tmpCands]; // number of candidates from the intersection
				tmpCands = (short) (stack[level].candidates & ~intersectionActCandSet);
				int anzExtra = sudoku.getSpecData().anzValues[tmpCands]; // number of candidates not drawn from the intersection
...
//                        tmpSet.andNot(blockActSet); // ???
						...
						if (globalStep.getCandidatesToDelete().size() > 0) {
							...
							int[] cands = sudoku.getSpecData().possibleValues[intersectionActCandSet];
	...
	private void getSetCandidates(SudokuSet srcSet1, SudokuSet srcSet2, short candSet, List<Candidate> dest) {
		...
		for (int i = 0; i < tmpSet.size(); i++) {
			...
			if ((sudoku.getCell(index) & candSet) != 0) {
				int[] cands = sudoku.getSpecData().possibleValues[sudoku.getCell(index) & candSet];
				for (int j = 0; j < cands.length; j++) {
	...
	private void checkCandidatesToDelete(SudokuSet tmpSet, short tmpCandSet) {
		...
		// ")");
		if (tmpSet.size() > 0 && sudoku.getSpecData().anzValues[tmpCandSet] > 0) {
			for (int i = 0; i < tmpSet.size(); i++) {
				int index = tmpSet.get(i);
				...
				int[] cands = sudoku.getSpecData().possibleValues[elimCandMask];
				for (int j = 0; j < cands.length; j++) {
