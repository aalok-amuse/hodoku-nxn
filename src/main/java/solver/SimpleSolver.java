...
public class SimpleSolver extends AbstractSolver {
	...
	/** Buffer for checking for locked Subsets. Sized per-spec via {@link #onSudokuChanged}. */
	...
	private short[] ipcMask = new short[17];
	...
	protected void onSudokuChanged(Sudoku2 sudoku) {
		singleFound = new boolean[sudoku.getLength()];
		int constrPerCell = sudoku.getConstraintsArr()[0].length;
		sameConstraint = new boolean[constrPerCell];
		foundConstraint = new boolean[constrPerCell];
		constraint = new int[constrPerCell];
	}
	...
	private SolutionStep findFullHouse(boolean all) {
		...
		while (queueIndex != -1) {
			...
			if (sudoku.getValue(index) == 0) {
				...
				for (int i = 0; i < sudoku.getConstraintsArr()[index].length; i++) {
					int constr = sudoku.getConstraintsArr()[index][i];
...
//                        step.setEntity(sudoku.getSpecData().constraintType[constr]);
//                        step.setEntityNumber(sudoku.getSpecData().constraintNumber[constr]);
						step.addValue(value);
						step.addIndex(index);
						if (all) {
							steps.add(step);
	...
	private SolutionStep findNakedXle(int anz, boolean lockedOnly) {
		...
		SolutionStep step = findNakedXleInEntity(sudoku.getBoxesArr(), anz, lockedOnly, !lockedOnly, true);
		if (step != null || lockedOnly) {
			return step;
		}
		step = findNakedXleInEntity(sudoku.getRowsArr(), anz, lockedOnly, !lockedOnly, true);
		if (step != null) {
			return step;
		}
		step = findNakedXleInEntity(sudoku.getColsArr(), anz, lockedOnly, !lockedOnly, true);
		return step;
	}
	...
	protected List<SolutionStep> findAllNakedXle() {
		...
		// now everything else
		for (int i = 2; i <= 4; i++) {
			findNakedXleInEntity(sudoku.getBoxesArr(), i, false, false, false);
			findNakedXleInEntity(sudoku.getRowsArr(), i, false, false, false);
			findNakedXleInEntity(sudoku.getColsArr(), i, false, false, false);
		}
		Collections.sort(steps);
		steps = oldList;
		return newList;
	...
	private SolutionStep findNakedXleInEntity(int[][] indices, int anz, boolean lockedOnly, boolean nakedOnly,
		...
		for (int entity = 0; entity < indices.length; entity++) {
			...
			for (int i = 0; i < indices[entity].length; i++) {
				int tmpAnz = sudoku.getSpecData().anzValues[sudoku.getCell(indices[entity][i])];
			...
			for (int i1 = 0; i1 < maxIndex - anz + 1; i1++) {
				...
				for (int i2 = i1 + 1; i2 < maxIndex - anz + 2; i2++) {
					...
					if (sudoku.getSpecData().anzValues[cell2] > anz) {
						continue;
					}
					if (anz == 2) {
						if (sudoku.getSpecData().anzValues[cell2] == anz) {
						...
						for (int i3 = i2 + 1; i3 < maxIndex - anz + 3; i3++) {
							...
							// take cell only if the combination of candidates
							// with the current result doesnt exceed anz
							if (sudoku.getSpecData().anzValues[cell3] > anz) {
								continue;
							}
							if (anz == 3) {
								if (sudoku.getSpecData().anzValues[cell3] == anz) {
								...
								for (int i4 = i3 + 1; i4 < maxIndex; i4++) {
									...
									// take cell only if the combination of candidates
									// with the current result doesnt exceed anz
									if (sudoku.getSpecData().anzValues[cell4] > anz) {
										continue;
									}
									if (sudoku.getSpecData().anzValues[cell4] == anz) {
										// step found!
	...
	private SolutionStep findHiddenSingle() {
		...
		while ((queueIndex = hsQueue.getSingle()) != -1) {
			...
			if (sudoku.getValue(index) == 0) {
				...
				for (int i = 0; i < sudoku.getConstraintsArr()[index].length; i++) {
					if (free[sudoku.getConstraintsArr()[index][i]][value] == 1) {
	...
	protected List<SolutionStep> findAllHiddenXle() {
		...
		steps.addAll(tmpSteps);
		for (int i = 2; i <= 4; i++) {
			findHiddenXleInEntity(2 * sudoku.getUnitCount(), sudoku.getBoxesArr(), i, false);
			findHiddenXleInEntity(0, sudoku.getRowsArr(), i, false);
			findHiddenXleInEntity(sudoku.getUnitCount(), sudoku.getColsArr(), i, false);
		}
		Collections.sort(steps);
		steps = oldList;
		return newList;
	...
	private SolutionStep findHiddenXle(int anz) {
		SudokuUtil.clearStepList(steps);
		SolutionStep step = findHiddenXleInEntity(2 * sudoku.getUnitCount(), sudoku.getBoxesArr(), anz, true);
		if (step != null) {
			return step;
		}
		step = findHiddenXleInEntity(0, sudoku.getRowsArr(), anz, true);
		if (step != null) {
			return step;
		}
		step = findHiddenXleInEntity(sudoku.getUnitCount(), sudoku.getColsArr(), anz, true);
		return step;
	}
	...
	private SolutionStep findHiddenXleInEntity(int constraintBase, int[][] indices, int anz, boolean onlyOne) {
		...
		for (int entity = 0; entity < indices.length; entity++) {
			...
			short candMask = 0;
			byte[][] free = sudoku.getFree();
			for (int i = 1; i <= sudoku.getUnitCount(); i++) {
				...
				if (actFree != 0 && actFree <= anz) {
					...
					candMask |= sudoku.getMasksArr()[i];
					ipcMask[i] = 0;
					for (int j = 0; j < sudoku.getUnitCount(); j++) {
						if ((sudoku.getCell(indices[entity][j]) & sudoku.getMasksArr()[i]) != 0) {
							ipcMask[i] |= sudoku.getMasksArr()[j + 1];
						}
					}
				}
			}
			if (sudoku.getSpecData().anzValues[candMask] < anz) {
				// not enough candidates
				continue;
			}
			// now check all possible combinations of candidates
			int[] candArr = sudoku.getSpecData().possibleValues[candMask];
			// first level: take all candidates
			for (int i1 = 0; i1 < candArr.length - anz + 1; i1++) {
				short cand1 = sudoku.getMasksArr()[candArr[i1]];
				short cell1 = ipcMask[candArr[i1]];
				// second level: always necessary.
				for (int i2 = i1 + 1; i2 < candArr.length - anz + 2; i2++) {
					short cand2 = (short) (cand1 | sudoku.getMasksArr()[candArr[i2]]);
					short cell2 = (short) (cell1 | ipcMask[candArr[i2]]);
					if (anz == 2) {
						if (sudoku.getSpecData().anzValues[cell2] == anz) {
							// step found!
							int[] tmp = sudoku.getSpecData().possibleValues[cell2];
						...
						for (int i3 = i2 + 1; i3 < candArr.length - anz + 3; i3++) {
							short cand3 = (short) (cand2 | sudoku.getMasksArr()[candArr[i3]]);
							short cell3 = (short) (cell2 | ipcMask[candArr[i3]]);
							if (anz == 3) {
								if (sudoku.getSpecData().anzValues[cell3] == anz) {
									// step found!
									int[] tmp = sudoku.getSpecData().possibleValues[cell3];
								...
								for (int i4 = i3 + 1; i4 < candArr.length; i4++) {
									short cand4 = (short) (cand3 | sudoku.getMasksArr()[candArr[i4]]);
									short cell4 = (short) (cell3 | ipcMask[candArr[i4]]);
									if (sudoku.getSpecData().anzValues[cell4] == anz) {
										// step found!
										int[] tmp = sudoku.getSpecData().possibleValues[cell4];
	...
	private SolutionStep findLockedCandidates(SolutionType type) {
		...
		if (type == SolutionType.LOCKED_CANDIDATES || type == SolutionType.LOCKED_CANDIDATES_2) {
			step = findLockedCandidatesInEntityN(0, sudoku.getRowsArr(), true);
			if (step != null) {
				return step;
			}
			step = findLockedCandidatesInEntityN(sudoku.getUnitCount(), sudoku.getColsArr(), true);
			if (step != null) {
				return step;
			}
		}
	...
	private SolutionStep findLockedCandidatesInEntityN(int constraintBase, int[][] indices, boolean onlyOne) {
		...
		SolutionStep step = null;
		byte[][] free = sudoku.getFree();
		for (int constr = 0; constr < sudoku.getUnitCount(); constr++) {
			// for every constraint check every candidate
			for (int cand = 1; cand <= sudoku.getUnitCount(); cand++) {
				...
				if (unitFree == 2 || unitFree == 3) {
					...
					for (int i = 0; i < indices[constr].length; i++) {
						...
						if ((cell & sudoku.getMasksArr()[cand]) == 0) {
					...
					int aktConstraint = -1;
					if (constraintBase == 2 * sudoku.getUnitCount()) {
						...
						step = createLockedCandidatesStep(SolutionType.LOCKED_CANDIDATES_1, cand, skipConstraint,
								sudoku.getAllUnitsArr()[aktConstraint]);
						if (onlyOne) {
							return step;
						} else {
							steps.add(step);
	...
	private SolutionStep createLockedCandidatesStep(SolutionType type, int cand, int skipConstraint, int[] indices) {
		...
		globalStep.setEntity(sudoku.getSpecData().constraintType[skipConstraint]);
		globalStep.setEntityNumber(sudoku.getSpecData().constraintNumber[skipConstraint]);
		// check all cells in the unit
		for (int i = 0; i < indices.length; i++) {
			int index = indices[i];
			if ((sudoku.getCell(index) & sudoku.getMasksArr()[cand]) != 0) {
				// cell holds the candidate
				if (sudoku.getConstraintsArr()[index][0] == skipConstraint || sudoku.getConstraintsArr()[index][1] == skipConstraint
						|| sudoku.getConstraintsArr()[index][2] == skipConstraint) {
...
//        System.out.println("constraint: " + Arrays.toString(constraint));
		...
		if (type.isHiddenSubset()) {
			...
			for (int i = 0; i < indices.length; i++) {
				...
				int[] candArray = sudoku.getSpecData().possibleValues[candsToDelete];
			...
			for (int i = 0; i < sameConstraint.length; i++) {
				...
				int[] cells = sudoku.getAllUnitsArr()[constraint[i]];
...
//                    System.out.println("  candArray: " + Arrays.toString(candArray));
		...
		int[] candArray = sudoku.getSpecData().possibleValues[cands];
