...
public class SingleDigitPatternSolver extends AbstractSolver {
	...
	/** For all entries in {@link #only2Constraints} the indices of the two cells. Sized per spec. */
	...
	protected void onSudokuChanged(Sudoku2 sudoku) {
		only2Indices = new int[2 * sudoku.getUnitCount()][2];
	}
	...
	protected List<SolutionStep> findAllEmptyRectangles() {
		...
		steps = newList;
		// ER tables are 9x9-specific — skip on other specs.
	...
	protected SolutionStep findEmptyRectangle() {
		// ER tables (erSets/erLines/erCols) are 9x9-specific — skip on other specs.
		if (sudoku.getUnitCount() != 9) {
			return null;
		}
		steps.clear();
	...
	private SolutionStep findEmptyRectanglesForCandidate(int cand, boolean onlyOne) {
		...
		for (int i = 0; i < sudoku.getSpecData().blockTemplates.length; i++) {
			...
			blockCands.and(sudoku.getSpecData().blockTemplates[i]);
			...
			for (int j = 0; j < erSets[i].length; j++) {
				...
				tmpSet.setAnd(blockCands, sudoku.getSpecData().rowTemplates[erLines[i][j]]);
				if (tmpSet.size() >= 2) {
					notEnoughCandidates = false;
				}
				tmpSet.andNot(sudoku.getSpecData().colTemplates[erCols[i][j]]);
				if (tmpSet.isEmpty()) {
					// not valid!
					continue;
				}
				...
				tmpSet.setAnd(blockCands, sudoku.getSpecData().colTemplates[erCols[i][j]]);
				if (tmpSet.size() >= 2) {
					notEnoughCandidates = false;
				}
				tmpSet.andNot(sudoku.getSpecData().rowTemplates[erLines[i][j]]);
				if (tmpSet.isEmpty()) {
					// not valid!
					continue;
				}
				...
				SolutionStep step = checkEmptyRectangle(cand, i, blockCands, sudoku.getRowsArr()[erLine],
						sudoku.getSpecData().rowTemplates, sudoku.getSpecData().colTemplates, erCol, false, onlyOne);
				if (onlyOne && step != null && !Options.getInstance().isAllowDualsAndSiamese()) {
					return step;
				}
				step = checkEmptyRectangle(cand, i, blockCands, sudoku.getColsArr()[erCol], sudoku.getSpecData().colTemplates,
						sudoku.getSpecData().rowTemplates, erLine, true, onlyOne);
				if (onlyOne && step != null && !Options.getInstance().isAllowDualsAndSiamese()) {
					return step;
				}
			}
	 ...
	 * @param LINE_TEMPLATES  Sudoku2.LINE_TEMPLATES/sudoku.getSpecData().colTemplates
	 * @param COL_TEMPLATES   sudoku.getSpecData().colTemplates/Sudoku2.LINE_TEMPLATES
	...
	private SolutionStep checkEmptyRectangle(int cand, int block, SudokuSet blockCands, int[] indices,
		...
		for (int i = 0; i < indices.length; i++) {
			...
			if (sudoku.isCandidate(index, cand)) {
				...
				int actCol = sudoku.colOf(index);
				if (lineColReversed) {
					actCol = sudoku.rowOf(index);
				}
				...
				if (tmpSet.size() == 2) {
					...
					int actLine = sudoku.rowOf(index2);
					if (lineColReversed) {
						actLine = sudoku.colOf(index2);
					}
					tmpSet.set(finder.getCandidates()[cand]);
					tmpSet.and(lineTemplates[actLine]);
					for (int j = 0; j < tmpSet.size(); j++) {
						int indexDel = tmpSet.get(j);
						if (sudoku.boxOf(indexDel) == block) {
							// cannot eliminate an ER candidate
							continue;
						}
						int colDel = sudoku.colOf(indexDel);
						if (lineColReversed) {
							colDel = sudoku.rowOf(indexDel);
						}
						if (colDel == firstCol) {
							// elimination found!
							globalStep.reset();
		...
		for (int cand = 1; cand <= sudoku.getUnitCount(); cand++) {
			...
			for (int constr = cStart; constr < cEnd; constr++) {
				if (free[constr][cand] == 2) {
					...
					int[] indices = sudoku.getAllUnitsArr()[constr];
					int candIndex = 0;
					for (int i = 0; i < indices.length; i++) {
			...
			for (int i = 0; i < constrCount; i++) {
				for (int j = i + 1; j < constrCount; j++) {
					...
					if (lines) {
						...
						if (sudoku.colOf(only2Indices[i][0]) == sudoku.colOf(only2Indices[j][0])) {
							found = true;
						}
						if (!found && sudoku.colOf(only2Indices[i][1]) == sudoku.colOf(only2Indices[j][1])) {
							found = true;
							otherIndex = 0;
						}
					} else {
						// must be in the same line
						if (sudoku.rowOf(only2Indices[i][0]) == sudoku.rowOf(only2Indices[j][0])) {
							found = true;
						}
						if (!found && sudoku.rowOf(only2Indices[i][1]) == sudoku.rowOf(only2Indices[j][1])) {
							found = true;
							otherIndex = 0;
						}
					}
					...
					if (lines
							&& sudoku.colOf(only2Indices[i][otherIndex]) == Sudoku2
									.getCol(only2Indices[j][otherIndex])
							|| !lines && sudoku.rowOf(only2Indices[i][otherIndex]) == Sudoku2
									.getRow(only2Indices[j][otherIndex])) {
						// step is X-Wing -> ignore
						continue;
					}
					// can something be eliminated?
					firstUnit.setAnd(finder.getCandidates()[cand], sudoku.getSpecData().buddies[only2Indices[i][otherIndex]]);
					firstUnit.and(sudoku.getSpecData().buddies[only2Indices[j][otherIndex]]);
		...
		for (int cand = 1; cand <= sudoku.getUnitCount(); cand++) {
			...
			for (int i = 0; i < constr1Count; i++) {
				for (int j = constr1Count; j < constr1Count + constr2Count; j++) {
					...
					// in only2indices[][1]
					if (sudoku.boxOf(only2Indices[i][0]) == sudoku.boxOf(only2Indices[j][0])) {
						// everything is as it should be -> do nothing
					} else if (sudoku.boxOf(only2Indices[i][0]) == sudoku.boxOf(only2Indices[j][1])) {
						int tmp = only2Indices[j][0];
						only2Indices[j][0] = only2Indices[j][1];
						only2Indices[j][1] = tmp;
					} else if (sudoku.boxOf(only2Indices[i][1]) == sudoku.boxOf(only2Indices[j][0])) {
						int tmp = only2Indices[i][0];
						only2Indices[i][0] = only2Indices[i][1];
						only2Indices[i][1] = tmp;
					} else if (sudoku.boxOf(only2Indices[i][1]) == sudoku.boxOf(only2Indices[j][1])) {
						int tmp = only2Indices[j][0];
						only2Indices[j][0] = only2Indices[j][1];
						only2Indices[j][1] = tmp;
					...
					int crossIndex = sudoku.cellOf(sudoku.rowOf(only2Indices[j][1]),
							sudoku.colOf(only2Indices[i][1]));
