...
public class SudokuStepFinder {
	 ...
	 */
	private SudokuSet[] setValueTemplates = new SudokuSet[17];
	/**
	 * One template per candidate with all positions from which the candidate can be
	 * eliminated immediately.
	 */
	private SudokuSet[] delCandTemplates = new SudokuSet[17];
	...
	private short[] candSets = new short[17];
	...
	private void initialize() {
		...
		// SudokuSets that are per-candidate (candidates, positions, candidatesAllowed,
		// setValueTemplates, delCandTemplates, emptyCells) are sized per spec by
		// ensureSetsSizedForSpec(), which is called whenever setSudoku() switches
		// the active sudoku. Wire the candidate-template list here — it's size-
		// independent — and leave the SudokuSet allocation entirely to setSudoku().
	...
	public void setSudoku(Sudoku2 sudoku) {
		...
		// Per-spec SudokuSets must be sized for the new sudoku's spec. Cheap if unchanged.
		ensureSetsSizedForSpec();
		// even if the reference is the same, the content could have been changed
		setSudokuDirty();
	}

	/** Length (cell count) the per-spec SudokuSets were last sized for. -1 = never. */
	private int sizedForLength = -1;

	/**
	 * Re-allocates the per-spec SudokuSets ({@link #candidates}, {@link #positions},
	 * {@link #candidatesAllowed}, {@link #emptyCells}, the templates) to match the
	 * active sudoku's BoardSpec. No-op when the spec hasn't changed.
	 */
	private void ensureSetsSizedForSpec() {
		if (sudoku == null) return;
		int len = sudoku.getLength();
		if (sizedForLength == len) return;
		BoardSpec spec = sudoku.getSpec();
		// (Re-)allocate per-candidate sets.
		for (int i = 0; i < candidates.length; i++) {
			candidates[i] = new SudokuSet(spec);
			positions[i] = new SudokuSet(spec);
			candidatesAllowed[i] = new SudokuSet(spec);
		}
		// Per-candidate templates.
		for (int i = 0; i < setValueTemplates.length; i++) {
			setValueTemplates[i] = new SudokuSet(spec);
			delCandTemplates[i] = new SudokuSet(spec);
		}
		// Singleton sets.
		emptyCells = new SudokuSet(spec);
		// `candSets` and other plain short[] fields aren't size-dependent.
		sizedForLength = len;
	}
	...
	private void initCandidates() {
		if (candidatesDirty) {
			...
			for (int i = 0; i < cells.length; i++) {
				int[] cands = sudoku.getSpecData().possibleValues[cells[i]];
	...
	private void initCandidatesAllowed() {
		if (candidatesAllowedDirty) {
			...
			for (int i = 0; i < values.length; i++) {
				if (values[i] != 0) {
					candidatesAllowed[values[i]].andNot(sudoku.getSpecData().buddies[i]);
					emptyCells.remove(i);
				}
			}
			for (int i = 1; i < candidatesAllowed.length; i++) {
...
//        }
		...
		if ((initLists && templatesListDirty) || (!initLists && templatesDirty)) {
			...
			SudokuSetBase[] templates = sudoku.getSpecData().templates;
			SudokuSetBase[] forbiddenPositions = new SudokuSetBase[17]; // eine 1 an jeder Position, an der Wert nicht
																		// mehr sein darf
...
//        SudokuSetBase temp = new SudokuSetBase();
	...
	private List<Als> doGetAlses(boolean onlyLargerThanOne) {
		...
		for (int i = 0; i < sudoku.getAllUnitsArr().length; i++) {
			for (int j = 0; j < sudoku.getAllUnitsArr()[i].length; j++) {
				indexSet.clear();
				candSets[0] = 0;
				checkAlsRecursive(0, j, sudoku.getAllUnitsArr()[i], alses, onlyLargerThanOne);
			}
		}
	...
	private void checkAlsRecursive(int anzahl, int startIndex, int[] indexe, List<Als> alses,
		...
		for (int i = startIndex; i < indexe.length; i++) {
			...
			if (sudoku.getSpecData().anzValues[candSets[anzahl]] - anzahl == 1) {
	...
	private List<RestrictedCommon> doGetRestrictedCommons(List<Als> alses, boolean withOverlap) {
		...
		for (int i = 0; i < alses.size(); i++) {
			...
			for (int j = start; j < alses.size(); j++) {
				...
				int[] prcs = sudoku.getSpecData().possibleValues[possibleRestrictedCommonsSet];
