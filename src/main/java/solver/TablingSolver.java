...
//    private SudokuSet alsBuddies = new SudokuSet(); // cells that can see all the cells of the als
	private SudokuSet[] alsEliminations = new SudokuSet[17]; // all cells with elminations for an als, sorted by
																// candidate
	...
	private void initialize() {
		if (!initialized) {
			onTable = new TableEntry[4352];
			offTable = new TableEntry[4352];
	...
	protected boolean checkKrakenTypeOne(SudokuSet fins, int index, int candidate) {
		for (int i = 0; i < fins.size(); i++) {
			int tableIndex = fins.get(i) * 17 + candidate;
	...
	protected boolean checkKrakenTypeTwo(SudokuSet indices, SudokuSet result, int startCandidate, int endCandidate) {
		...
		for (int i = 0; i < indices.size(); i++) {
			int tableIndex = indices.get(i) * 17 + startCandidate;
	...
	protected Chain getKrakenChain(int startIndex, int startCandidate, int endIndex, int endCandidate) {
		globalStep.reset();
		resetTmpChains();
		addChain(onTable[startIndex * 17 + startCandidate], endIndex, endCandidate, false);
...
//        }
	...
	private void checkForcingChains() {
		...
		checkAllChainsForHouse(sudoku.getSpecData().rowTemplates);
		checkAllChainsForHouse(sudoku.getSpecData().colTemplates);
		checkAllChainsForHouse(sudoku.getSpecData().blockTemplates);
	}
		...
		if (houseSets == null) {
			...
			for (int i = 0; i < sudoku.getLength(); i++) {
				...
				for (int j = 0; j < cands.length; j++) {
					entryList.add(onTable[i * 17 + cands[j]]);
				}
				// do the checks
				...
				for (int j = 1; j < finder.getCandidates().length; j++) {
					...
					if (!tmpSet.isEmpty()) {
						...
						for (int k = 0; k < tmpSet.size(); k++) {
							entryList.add(onTable[tmpSet.get(k) * 17 + j]);
						}
						// do the checks
						checkEntryList(entryList);
					}
...
//        System.out.println("replaceorcopystep: " + globalStep.toString(2));
	...
	private void checkOneChain(TableEntry entry) {
		...
		checkHouseSet(entry, sudoku.getSpecData().rowTemplates, Sudoku2.ROW);
		checkHouseSet(entry, sudoku.getSpecData().colTemplates, Sudoku2.COL);
		checkHouseSet(entry, sudoku.getSpecData().blockTemplates, Sudoku2.BLOCK);
		...
		checkHouseDel(entry, sudoku.getSpecData().rowTemplates, Sudoku2.ROW);
		checkHouseDel(entry, sudoku.getSpecData().colTemplates, Sudoku2.COL);
		checkHouseDel(entry, sudoku.getSpecData().blockTemplates, Sudoku2.BLOCK);
	}
	...
	private void checkAics(TableEntry[] tables) {
		for (int i = 0; i < tables.length; i++) {
			...
			SudokuSetBase buddies = sudoku.getSpecData().buddies[startIndex];
			for (int j = 1; j < tables[i].index; j++) {
				...
				if (startCandidate == tables[i].getCandidate(j)) {
					// check Type 1
					tmpSet.set(buddies);
					tmpSet.and(sudoku.getSpecData().buddies[tables[i].getCellIndex(j)]);
...
//                tmpSet.set(sudoku.getSpecData().buddies[Chain.getSCellIndex(nlChain[1])]);
//                tmpSet.and(sudoku.getSpecData().buddies[Chain.getSCellIndex(nlChain[nlChainIndex - 1])]);
//                tmpSet.and(finder.getCandidates()[startCandidate]);
//                if (tmpSet.size() > 1) {
...
//                        (i > 0) && (!Chain.isSStrong(nlChain[i]) && Chain.getSCellIndex(nlChain[i - 1]) != Chain.getSCellIndex(nlChain[i])))) {
				if ((i > 0) && (!Chain.isSStrong(nlChain[i])
					...
					if (Chain.getSNodeType(nlChain[i]) == Chain.ALS_NODE) {
						...
						if (isForceExit) {
							...
							tmpSet1.set(sudoku.getSpecData().buddies[nextCellIndex]);
	...
	private void checkAic(TableEntry entry, int entryIndex) {
		...
		if (startCandidate == endCandidate) {
			...
			tmpSet.set(sudoku.getSpecData().buddies[startIndex]);
			tmpSet.and(sudoku.getSpecData().buddies[endIndex]);
			tmpSet.and(finder.getCandidates()[startCandidate]);
			if (tmpSet.size() > 1) {
				for (int i = 0; i < tmpSet.size(); i++) {
					if (tmpSet.get(i) != startIndex) {
	...
	private void fillTables() {
		...
		if (chainsOnly) {
			...
			for (int i = 0; i < sudoku.getCells().length; i++) {
				...
				for (int j = 1; j <= sudoku.getUnitCount(); j++) {
					...
					int cand = j;
					onTable[i * 17 + cand].addEntry(i, cand, true);
					offTable[i * 17 + cand].addEntry(i, cand, false);
...
//                        if (sudoku.getAnzCandidates(i) == 2) {
					...
					for (int constrIndex = 0; constrIndex < sudoku.getConstraintsArr()[i].length; constrIndex++) {
						...
						// get the candidates
						tmpSet.set(tmpSet1);
						tmpSet.and(sudoku.getSpecData().allConstraintsTemplates[constr]);
						if (tmpSet.isEmpty()) {
							// no candidates left...
							continue;
						}
						for (int k = 0; k < tmpSet.size(); k++) {
							...
							onTable[i * 17 + cand].addEntry(tmpSet.get(k), cand, false);
						}
						if (anzCands == 2) {
							// strong link: if cand is OFF, the other candidate has to be ON
							offTable[i * 17 + cand].addEntry(tmpSet.get(0), cand, true);
						}
					}
				}
			}
...
//            }
				...
				for (int j = 0; j < cands.length; j++) {
					...
					getTableEntry(onTable[i * 17 + cand], i, cand, true);
					// candidate is OFF
					sudoku.set(savedSudoku);
					simpleFinder.setSudoku(sudoku);
					getTableEntry(offTable[i * 17 + cand], i, cand, false);
				}
			}
			sudoku.set(savedSudoku);
		}
		...
		for (int i = 0; i < groupNodes.size(); i++) {
			...
			if (!tmpSet.isEmpty()) {
				...
				for (int j = 0; j < tmpSet.size(); j++) {
					...
					TableEntry tmp = onTable[index * 17 + gn.cand];
				...
				tmpSet1.set(tmpSet);
				tmpSet1.and(sudoku.getSpecData().blockTemplates[gn.block]);
				if (!tmpSet1.isEmpty() && tmpSet1.size() == 1) {
					offEntry.addEntry(tmpSet1.get(0), gn.cand, true);
					TableEntry tmp = offTable[tmpSet1.get(0) * 17 + gn.cand];
					tmp.addEntry(gn.index1, gn.index2, gn.index3, Chain.GROUP_NODE, gn.cand, true, 0, 0, 0, 0, 0, 0);
				}
				tmpSet1.set(tmpSet);
				if (gn.line != -1) {
					tmpSet1.and(sudoku.getSpecData().rowTemplates[gn.line]);
				} else {
					tmpSet1.and(sudoku.getSpecData().colTemplates[gn.col]);
				}
				if (!tmpSet1.isEmpty() && tmpSet1.size() == 1) {
					offEntry.addEntry(tmpSet1.get(0), gn.cand, true);
					TableEntry tmp = offTable[tmpSet1.get(0) * 17 + gn.cand];
			...
			if (lineAnz == 1) {
				...
				tmpSet.set(sudoku.getSpecData().rowTemplates[gn.line]);
			...
			if (colAnz == 1) {
				...
				tmpSet.set(sudoku.getSpecData().colTemplates[gn.col]);
				tmpSet.and(finder.getCandidates()[gn.cand]);
				tmpSet.andNot(gn.indices);
				tmpSet.andNot(gn2.indices);
				if (tmpSet.isEmpty()) {
			...
			if (blockAnz == 1) {
				...
				tmpSet.set(sudoku.getSpecData().blockTemplates[gn.block]);
				tmpSet.and(finder.getCandidates()[gn.cand]);
				tmpSet.andNot(gn.indices);
				tmpSet.andNot(gn2.indices);
				if (tmpSet.isEmpty()) {
...
//            als.getBuddies(alsBuddies);
			...
			for (int j = 1; j <= sudoku.getUnitCount(); j++) {
				...
				for (int k = 1; k <= sudoku.getUnitCount(); k++) {
				...
				for (int k = 0; k < tmpSet.size(); k++) {
					...
					TableEntry tmp = onTable[actIndex * 17 + j];
...
//                        offEntry.addEntry(gAct.index1, gAct.index2, gAct.index3, Chain.GROUP_NODE, k, false, 0, 0, 0, 0, 0);
				...
				for (int k = 0; k < alses.size(); k++) {
					...
					for (int l = 1; l <= sudoku.getUnitCount(); l++) {
...
//                        offEntry.addEntry(cellIndex, tmpSet1.get(0), true);
	...
	private void setCell(int cellIndex, int cand, TableEntry entry, boolean getRetIndices, boolean nakedSingle) {
		...
		tmpSet.and(sudoku.getSpecData().buddies[cellIndex]);
		...
		int entityNumberFree = sudoku.getFree()[sudoku.getConstraintsArr()[cellIndex][0]][cand];
		int dummy = sudoku.getFree()[sudoku.getConstraintsArr()[cellIndex][1]][cand];
		if (dummy < entityNumberFree) {
			entityType = Sudoku2.COL;
			entityNumberFree = dummy;
		}
		dummy = sudoku.getFree()[sudoku.getConstraintsArr()[cellIndex][2]][cand];
		if (dummy < entityNumberFree) {
		...
		if (getRetIndices) {
			...
			if (nakedSingle) {
				...
				if (entityType == Sudoku2.ROW) {
					getRetIndicesForHouse(cellIndex, cand, sudoku.getSpecData().rowTemplates[sudoku.rowOf(cellIndex)], entry);
				} else if (entityType == Sudoku2.COL) {
					getRetIndicesForHouse(cellIndex, cand, sudoku.getSpecData().colTemplates[sudoku.colOf(cellIndex)], entry);
				} else {
					getRetIndicesForHouse(cellIndex, cand, sudoku.getSpecData().blockTemplates[sudoku.boxOf(cellIndex)], entry);
				}
			}
...
//            }
			...
			for (int j = 1; j < dest.entries.length; j++) {
				...
				int srcTableIndex = dest.getCellIndex(j) * 17 + dest.getCandidate(j);
