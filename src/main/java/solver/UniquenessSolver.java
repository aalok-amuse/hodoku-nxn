...
public class UniquenessSolver extends AbstractSolver {
	...
	private SolutionStep getBugPlus1() {
		...
		// still here?
		if (sudoku.isCandidate(index3, cand3) && sudoku.getConstraintsArr()[index3][0] == bugConstraints[0]
				&& sudoku.getConstraintsArr()[index3][1] == bugConstraints[1]
				&& sudoku.getConstraintsArr()[index3][2] == bugConstraints[2]) {
			// ok it IS a BUG+1
			// don't call initStep()!
			globalStep.reset();
...
//        System.out.println("index11 = " + index11 + ", cand1 = " + cand1 + ", cand2 = " + cand2);
		...
		int line11 = sudoku.rowOf(index11);
		int col11 = sudoku.colOf(index11);
		int block11 = sudoku.boxOf(index11);
...
//        System.out.println("allowedCand2: " + allowedCand2);
		int[] blockIndices = sudoku.getBoxesArr()[sudoku.boxOf(index11)];
		for (int i = 0; i < blockIndices.length; i++) {
			if (blockIndices[i] == index11) {
				continue;
			}
			int index12 = blockIndices[i];
			if (line11 != sudoku.rowOf(index12) && col11 != sudoku.colOf(index12)) {
			...
			if ((!avoidable && (sudoku.getValue(index12) == 0 && ((!allowMissing && (cell11 & cell12) == cell11)
				...
				boolean isLines = line11 == sudoku.rowOf(index12);
				// get the units that have to hold the opposite side of the rectangle
				int[] unit11 = sudoku.getAllUnitsArr()[isLines ? sudoku.colOf(index11) + 9 : sudoku.rowOf(index11)];
				int[] unit12 = sudoku.getAllUnitsArr()[isLines ? sudoku.colOf(index12) + 9 : sudoku.rowOf(index12)];
				// check the units: two adequat cells at the same indices in a different block
				for (int j = 0; j < unit11.length; j++) {
					if (sudoku.boxOf(unit11[j]) == block11) {
...
//        }
		...
		short urMask = (short) (sudoku.getMasksArr()[cand1] | sudoku.getMasksArr()[cand2]);
		...
		if (twoSize == 2 || twoSize == 1) {
			...
			for (int i = 0; i < additionalCandidates.size(); i++) {
				...
				if (sudoku.getSpecData().anzValues[addMask] > 1) {
					break;
				}
				tmpSet.and(sudoku.getSpecData().buddies[index3]);
			}
			if (sudoku.getSpecData().anzValues[addMask] == 1) {
				// ok, valid UR 2 or UR 5
				// get cells that can see all cells with the additional candidate
				int addCand = sudoku.getSpecData().candFromMask[addMask];
				...
				if (!tmpSet.isEmpty()) {
					...
					if (additionalCandidates.size() == 3 || (sudoku.rowOf(i1) != sudoku.rowOf(i2)
							&& sudoku.colOf(i1) != sudoku.colOf(i2))) {
		...
		if (twoSize == 2) {
			...
			int i1 = additionalCandidates.get(0);
			int i2 = additionalCandidates.get(1);
			if (sudoku.rowOf(i1) == sudoku.rowOf(i2)) {
				step = checkUniqueness3(Sudoku2.ROW, sudoku.getRowsArr()[sudoku.rowOf(i1)], u3Cands, urMask, searchType,
						onlyOne);
				if (step != null && onlyOne) {
			...
			if (sudoku.colOf(i1) == sudoku.colOf(i2)) {
				step = checkUniqueness3(Sudoku2.COL, sudoku.getColsArr()[sudoku.colOf(i1)], u3Cands, urMask, searchType,
						onlyOne);
				if (step != null && onlyOne) {
					// if steps should be cached that was already done
					return step;
			...
			if (sudoku.boxOf(i1) == sudoku.boxOf(i2)) {
				step = checkUniqueness3(Sudoku2.BLOCK, sudoku.getBoxesArr()[sudoku.boxOf(i1)], u3Cands, urMask,
						searchType, onlyOne);
				if (step != null && onlyOne) {
					// if steps should be cached that was already done
					return step;
...
//            System.out.println("check UR4 " + additionalCandidates + "/" + cand1 + "/" + cand2);
			int i1 = additionalCandidates.get(0);
			int i2 = additionalCandidates.get(1);
			if ((sudoku.rowOf(i1) == sudoku.rowOf(i2)) || (sudoku.colOf(i1) == sudoku.colOf(i2))) {
				// get all cells that can see both cells with additional candidates
				tmpSet.setAnd(sudoku.getSpecData().buddies[i1], sudoku.getSpecData().buddies[i2]);
...
//                System.out.println("   delCand: " + delCand);
		...
		if (twoSize == 2) {
			...
			if ((sudoku.rowOf(i1) != sudoku.rowOf(i2)) && (sudoku.colOf(i1) != sudoku.colOf(i2))) {
				...
				tmpSet.set(sudoku.getSpecData().rowTemplates[sudoku.rowOf(i1)]);
				tmpSet.or(sudoku.getSpecData().colTemplates[sudoku.colOf(i1)]);
				tmpSet.or(sudoku.getSpecData().rowTemplates[sudoku.rowOf(i2)]);
				tmpSet.or(sudoku.getSpecData().colTemplates[sudoku.colOf(i2)]);
...
//            System.out.println("check AR " + index21 + "/" + index22);
			...
			// check for deletions
			tmpSet.set(sudoku.getSpecData().buddies[index21]);
			tmpSet.and(sudoku.getSpecData().buddies[index22]);
	...
	private SolutionStep checkHiddenRectangle(int cornerIndex, SolutionType type, boolean onlyOne) {
		...
		int lineC = sudoku.rowOf(cornerIndex);
		int colC = sudoku.colOf(cornerIndex);
		int i1 = additionalCandidates.get(0);
		int i2 = additionalCandidates.get(1);
		int line1 = sudoku.rowOf(i1);
		if (line1 == lineC) {
			line1 = sudoku.rowOf(i2);
		}
		int col1 = sudoku.colOf(i1);
		if (col1 == colC) {
			col1 = sudoku.colOf(i2);
		}
...
//        System.out.println("check " + line + "/" + col + "/" + cand1 + "/" + cand2);
		...
		tmpSet.and(sudoku.getSpecData().rowTemplates[line]);
...
//        System.out.println(tmpSet);
		...
		tmpSet.set(finder.getCandidates()[cand1]);
		tmpSet.and(sudoku.getSpecData().colTemplates[col]);
		tmpSet.andNot(tmpSet1);
//        System.out.println(tmpSet);
		if (!tmpSet.isEmpty()) {
			return null;
		...
		int delIndex = sudoku.cellOf(line, col);
...
//        System.out.println("search U3: " + Arrays.toString(sudoku.getSpecData().possibleValues[u3Cands]) + "/" + Arrays.toString(sudoku.getSpecData().possibleValues[urMask]));
	...
	private SolutionStep checkUniqueness3Recursive(int type, int[] unit, SudokuSet u3Indices, short candsIncluded,
		...
		for (int i = startIndex; i < u3Indices.size(); i++) {
			...
			if (type != Sudoku2.BLOCK || !isSameLineOrCol(aktIndices)) {
				...
				if (sudoku.getSpecData().anzValues[aktCands] == (aktIndices.size() - 1)) {
//                    System.out.println("UR3 found (" + type + "):");
//                    System.out.println("  cands: " + Arrays.toString(sudoku.getSpecData().possibleValues[aktCands]));
...
//                            System.out.println("   unit[j] = " + unit[j] + ", value = " + sudoku.getValue(unit[j]) + ", delCands = " + Integer.toBinaryString(delCands));
							if (sudoku.getSpecData().anzValues[delCands] == 0) {
								// no candidates to delete
								continue;
							}
							int[] delCandsArray = sudoku.getSpecData().possibleValues[delCands];
...
//                                System.out.println("      ctd: " + unit[j] + "/" + delCandsArray[k]);
					...
					if (globalStep.getCandidatesToDelete().size() > 0) {
						// write the fins
						int[] aktCandsArray = sudoku.getSpecData().possibleValues[aktCands];
...
//                        }
						...
						if (type == Sudoku2.ROW || type == Sudoku2.COL) {
							...
							if (block != -1) {
								int[] unit1 = sudoku.getBoxesArr()[block];
...
//                                            System.out.println("      ctd: " + unit1[j] + "/" + delCandsArray[k]);
	...
	private int getBlockForCheck3(SudokuSet aktIndices) {
		...
		int block = sudoku.boxOf(aktIndices.get(0));
		for (int i = 1; i < aktIndices.size(); i++) {
			if (sudoku.boxOf(aktIndices.get(i)) != block) {
				return -1;
			}
		}
		return block;
	...
	private boolean isSameLineOrCol(SudokuSet aktIndices) {
		...
		int line = sudoku.rowOf(aktIndices.get(0));
		int col = sudoku.colOf(aktIndices.get(0));
		for (int i = 1; i < aktIndices.size(); i++) {
			if (sudoku.rowOf(aktIndices.get(i)) != line) {
				sameLine = false;
			}
			if (sudoku.colOf(aktIndices.get(i)) != col) {
				sameCol = false;
			}
		}
	...
	private void initCheck(int[] indices) {
		...
		short mask = (short) ~(sudoku.getMasksArr()[cand1] | sudoku.getMasksArr()[cand2]);
