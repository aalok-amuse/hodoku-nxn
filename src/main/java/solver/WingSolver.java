...
public class WingSolver extends AbstractSolver {
	...
	/** The indices of all bivalue cells in the current sudoku (for XY-Wing). Sized per spec. */
	private int[] biCells;
	/** The indices of all trivalue cells in the current sudoku (for XYZ-Wing). Sized per spec. */
	...
	protected void onSudokuChanged(Sudoku2 sudoku) {
		biCells = new int[sudoku.getLength()];
		triCells = new int[sudoku.getLength()];
	}
	...
	private SolutionStep getWing(boolean xyz, boolean onlyOne) {
		...
		for (int i = 0; i < endIndex; i++) {
			for (int j = xyz ? 0 : i + 1; j < biValueCount; j++) {
				...
				if (sudoku.getSpecData().anzValues[sudoku.getCell(biTri[i]) | sudoku.getCell(biCells[j])] != 3) {
					// cant become a wing
					continue;
				}
				for (int k = j + 1; k < biValueCount; k++) {
					...
					if (sudoku.getSpecData().anzValues[cell1 | cell2 | cell3] != 3) {
					...
					for (int tries = 0; tries < maxTries; tries++) {
						...
						if (!sudoku.getSpecData().buddies[index1].contains(index2) || !sudoku.getSpecData().buddies[index1].contains(index3)) {
						...
						if (sudoku.getSpecData().anzValues[cell] != 1) {
							// no wing, sorry
							continue;
						}
						int candZ = sudoku.getSpecData().candFromMask[cell];
						// are there candidates that can see the pincers?
						elimSet.setAnd(sudoku.getSpecData().buddies[index2], sudoku.getSpecData().buddies[index3]);
						elimSet.and(finder.getCandidates()[candZ]);
						if (xyz) {
							// the pivot as well
							elimSet.and(sudoku.getSpecData().buddies[index1]);
						}
						if (elimSet.isEmpty()) {
	...
	private SolutionStep getWWing(boolean onlyOne) {
		for (int i = 0; i < sudoku.getCells().length; i++) {
			...
			preCalcSet1.setAnd(sudoku.getSpecData().buddies[i], finder.getCandidates()[cand1]);
			preCalcSet2.setAnd(sudoku.getSpecData().buddies[i], finder.getCandidates()[cand2]);
			...
			for (int j = i + 1; j < sudoku.getCells().length; j++) {
				...
				elimSet.setAnd(preCalcSet1, sudoku.getSpecData().buddies[j]);
				if (!elimSet.isEmpty()) {
				...
				elimSet.setAnd(preCalcSet2, sudoku.getSpecData().buddies[j]);
				if (!elimSet.isEmpty()) {
			...
			if (free[constr][cand2] == 2) {
				...
				for (int i = 0; i < indices.length; i++) {
					...
					if (aktIndex != index1 && aktIndex != index2 && sudoku.isCandidate(aktIndex, cand2)) {
						...
						if (sudoku.getSpecData().buddies[aktIndex].contains(index1)) {
							sees1 = true;
							wIndex1 = aktIndex;
						} else if (sudoku.getSpecData().buddies[aktIndex].contains(index2)) {
							sees2 = true;
							wIndex2 = aktIndex;
						}
					}
