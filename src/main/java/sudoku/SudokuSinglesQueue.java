...
public class SudokuSinglesQueue implements Cloneable {
	/** The indices of newly detected Singles. Sized for 9x9 by default; resizable. */
	private int[] indices = new int[Sudoku2.LENGTH * 3];
	/** The values of newly detected Singles. Sized for 9x9 by default; resizable. */
...
//        System.out.println("    add Single " + index + "/" + value + " at pos " + putIndex);
		if (putIndex >= indices.length) {
			// Grow on demand so non-9x9 boards don't overflow. Double-and-copy keeps amortised cost low.
			int newLen = indices.length * 2;
			int[] ni = new int[newLen]; int[] nv = new int[newLen];
			System.arraycopy(indices, 0, ni, 0, indices.length);
			System.arraycopy(values, 0, nv, 0, values.length);
			indices = ni; values = nv;
		}
	...
	public void deleteHiddenSingle(int constraint, int value) {
		deleteHiddenSingle(constraint, value, Sudoku2.CONSTRAINTS);
	}

	/**
	 * Spec-aware variant: caller supplies the per-cell constraints array
	 * (typically {@code sudoku.getConstraintsArr()}). The 1-arg overload above
	 * delegates here with the legacy 9x9 statics for backwards compatibility.
	 *
	 * @param constraint  unit index to remove a matching single for
	 * @param value       digit
	 * @param constraints per-cell constraint-unit indices for this sudoku's spec
	 */
	public void deleteHiddenSingle(int constraint, int value, int[][] constraints) {
		for (int i = getIndex; i < putIndex; i++) {
			int actIndex = indices[i];
			if (values[i] != value) continue;
			int[] cs = constraints[actIndex];
			boolean match = false;
			for (int k = 0; k < cs.length; k++) {
				if (cs[k] == constraint) { match = true; break; }
			}
			if (match) {
				for (int j = i + 1; j < putIndex; j++) {
					indices[j - 1] = indices[j];
